...
async def entrypoint(ctx: JobContext):
    ...
    system_message = await evolving_agent.gemini_llm.generate_system_message(
        user_id, personality, conversation_count
    )
    
    logger.info(f"📝 Gemini-generated system prompt created for user {user_id}")
    ...
    # Create chat context with system message using the correct API
    from livekit.agents.llm import ChatMessage
    # ChatMessage expects content to be a list, not a string
    system_message_obj = ChatMessage(role="system", content=[{"type": "text", "text": system_message}])
    chat_context = llm.ChatContext(messages=[system_message_obj])
    ...
    assistant = VoiceAssistant(
        ...
        # 💬 CHAT CONTEXT WITH GEMINI-GENERATED SYSTEM MESSAGE
